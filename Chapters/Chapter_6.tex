\section{Meson spectrometry}
\label{sec:hadron-spectrometry}

The color degree of freedom introduced in Sect.~\ref{subsec:color-experiment} completes the
...
the quark picture and of the short- and long-distance behavior of the strong force. This
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{Figures/jpsi_discovery.png}
  ...
  at SPEAR in three annihilation channels, fixing $J^{PC}=1^{--}$. Adapted from
  the lecture slides (week 21, "Summary: $J/\psi$ discovery (1974)").}
...
approximate, but do not exactly reproduce, the ideal Breit--Wigner behavior
...
particles in the center-of-mass frame, $E$ the center-of-mass energy, and $J$ the resonance
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/argand_delta.png}
  \caption{Left: Argand diagram of the resonant amplitude
  \eqref{eq:amplitude-phaseshift}, tracing the unitarity circle with the
  resonance point $R$ at the top and the phase shift $\delta$. Right: the
  measured $P_{33}$ ($\Delta(1232)$) partial-wave amplitude in $\pi N$
  scattering, real and imaginary parts versus $W$, illustrating departures
  from the ideal circle away from resonance. Adapted from the lecture slides
  (week 21, "Summary: Resonance").}
...
$c\bar c$ must annihilate completely, and no color can be transmitted from the color-singlet
$c\bar c$ state to the color-singlet final hadrons except through a colorless combination
...
with color neutrality requires at least three gluons, $n \geq 3$. Each of these gluons is
...
$k = 1\,\mathrm{GeV\,fm^{-1}}$ \cite[Ch.~9]{Amsler2018}. The color factor $4/3$ arises from
the $\mathrm{SU(3)}_{C}$ structure of one-gluon exchange between a color and an anticolor
...
\begin{figure}[htbp]
    ...
    Coulomb-like term alone; the linear term produces the rising confining behavior at
    large $r$.}
...
familiar $1/r^2$ force, the color field between a quark and an antiquark is squeezed into a
...
breaks, and two color-singlet hadrons fly apart. This is why isolated color charges are
...
linear term of \eqref{eq:cornell-potential} implies for the large-$r$ behavior of the
...
Sect.~\ref{subsec:qq-potential}. The states are labeled spectroscopically as
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{Figures/charmonium_positronium.png}
  ...
  $e^+e^-$ system and a strongly bound $c\bar c$ system. The $D\bar D$
  threshold is indicated on the charmonium side; states above it are broad.
  Adapted from the lecture slides (week 21, "Resulting spectrum").}
...
$\chi_{c2}(1P) = 3556.17 \pm 0.07\,\mathrm{MeV}$, matching the tabulated triplet masses
$3510.7$ and $3556.2\,\mathrm{MeV}$; the $\chi_{c0}$ sits well below them, at
$3414.7\,\mathrm{MeV}$ \cite[Ch.~9]{Amsler2018}.

The $h_c(1P)$ itself had a more troubled history than this success suggests. E760
reported a state in the isospin-violating $J/\psi\,\pi^0$ final state at the expected
mass, but \emph{the claim was withdrawn} when the upgraded detector operated by E785
saw no $J/\psi\,\pi^0$ signal.%
\footnote{The lecture slides show this E760 $p\bar p \to J/\psi\,\pi^0$ measurement
labeled as the $h_c(1P)$, citing Phys.\ Rev.\ Lett.\ \textbf{69} (1992) 2337. Amsler
\cite[Ch.~9]{Amsler2018} records that this claim did not survive; the established
observation is the BESIII one described below.} What survived from the $p\bar p$ programme was a narrow
($<1\,\mathrm{MeV}$) $15$-event structure at $3526\,\mathrm{MeV}$ seen in the E1
transition to $\eta_c(1S)\gamma$. Comprehensive data came only later, from the BESIII
detector at the Beijing collider BEPC, which used the isospin-violating transition
$\psi(2S) \to \pi^0 h_c(1P)$ (branching ratio $\sim 10^{-3}$) followed by
$h_c(1P) \to \gamma\,\eta_c(1S)$ (branching ratio $\simeq 50\%$), reconstructing the
$\eta_c(1S)$ in sixteen decay channels and recoiling against the $\pi^0$. The signal
peaks at $3525.3 \pm 0.2\,\mathrm{MeV}$ with a width of $0.7\,\mathrm{MeV}$, lying
exactly at the center of gravity of the $\chi_{cJ}$ triplet
\cite[Ch.~9]{Amsler2018}. The same charmonium states
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{Figures/charmonium_radiative_scan.png}
  \caption{Probing the charmonium spectrum. Top left: inclusive photon
  spectrum from $e^+e^-\to\psi(2S)\to\gamma X$ (Crystal Ball), with the
  numbered radiative lines corresponding to the transitions in the level
  diagram (top right), color-coded by $E1$ radiative, $M1$ radiative,
  hadronic and two-body electromagnetic transitions. Bottom: $p\bar p$
  formation scans of the $\chi_{c1}(1P)$ and $\chi_{c2}(1P)$ (E760/E835),
  whose narrow widths are resolved by the precise beam momentum. Adapted
  from the lecture slides (week 21, "Charmonium Spectrum in Radiative
  Decays" and "Charmonium Spectrum in Beam Scans").}
...
$b\bar b$ system, observed in $e^+e^-$ annihilation at center-of-mass energies near
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{Figures/bottomonium_comparison.png}
  \caption{Top: the $R$-ratio in the bottomonium region, showing the narrow
  $\Upsilon(1S,2S,3S)$ resonances below, and the broad $\Upsilon(4S)$ at, the
  $B\bar B$ threshold. Bottom: positronium, charmonium and bottomonium level
  schemes compared on a common grid, illustrating the similar low-lying
  structure and the differing high-lying spacings. Adapted from the lecture
  slides (week 21, "R-plot revisited - Bottomonium" and "Comparison of
  $e^+e^-$ vs $c\bar c$ vs $b\bar b$").}
...
remarkably small: Amsler quotes $m(X) = 3871.7 \pm 0.2\,\mathrm{MeV}$ with only an
upper limit $\Gamma < 1.2\,\mathrm{MeV}$ \cite[Ch.~16]{Amsler2018}, while the sharper
values above are the later LHCb/PDG determinations. The dipion substructure of the
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{Figures/x3872_discovery.png}
  \caption{The discovery of the $X(3872)$. Left: the $J/\psi\,\pi^+\pi^-$
  invariant mass from Belle in $B^+ \to K^+ X$, $X\to J/\psi\,\pi^+\pi^-$.
  Right: measurements of $m(X)$ by CDF, BaBar, D0, Belle and LHCb, compared
  with the $D^0\bar D^{*0}$ threshold (shaded band). Adapted from the lecture
  slides (week 21, "A surprise at higher energies: X(3872)" and "X(3872):
  Observations").}
  \label{fig:x3872-discovery}
\end{figure}
...
states: the compact \emph{tetraquark}, a color-bound object formed from a $Qq$ diquark and a
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{Figures/multiquark_cartoons.png}
  \caption{Three generic pictures of multiquark exotics (top to bottom): the
  compact tetraquark, a color-bound $Qq$--$\bar Q\bar q$ pair; the
  hadro-quarkonium, a compact $Q\bar Q$ core surrounded by light quarks $q$,
  $\bar q$; and the hadronic molecule, an extended object made of a $\bar Q q$
  meson bound to a $Q\bar q$ meson. Adapted from the lecture slides (week 21,
  "Summary: Multiquark Exotics").}
  \label{fig:multiquark-cartoons}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{Figures/xyz_spectrum.png}
  \caption{The $XYZ$ states populating the charmonium region, plotted as
  invariant mass versus $J^{PC}$. Established $c\bar c$ levels (black/blue)
  are shown alongside the exotic $X$, $Y$ and $Z$/$T$ candidates (red/purple),
  many of which cluster near the open-charm thresholds $2m_D$, $2m_{D^*}$ and
  $m_D+m_{D^*}$ (dotted lines). Adapted from the lecture slides (week 21,
  "The XYZ spectrum").}
...
containing two charm quarks of the same flavor, and it lies just below the $D^{*+}D^0$
...
once QCD allowed color-singlet combinations beyond $q\bar q$ and $qqq$; the difficulty is
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{Figures/tetraquark_lhcb_cms.png}
  \caption{Left: the CMS di-$J/\psi$ invariant-mass spectrum, showing the
  three fully charmed tetraquark candidates $T_{c\bar c c\bar c}(6600)$,
  $T_{c\bar c c\bar c}(6900)$ and $T_{c\bar c c\bar c}(7100)$. Right: the LHCb
  $D^0D^0\pi^+$ invariant-mass spectrum showing the $T_{cc}^+$ peak just below
  the $D^{*+}D^0$ threshold. Adapted from the lecture slides (week 21,
  "Di-$J/\psi$" and "A new type of exotic").}
  \label{fig:tetraquark-lhcb-cms}
\end{figure}
